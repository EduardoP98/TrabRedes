    ...
    def take_action(self, action_name):
        ...
        if action_name == actions.QUIT_ACTION or len(action_name) == 0:
            ...
            if self.nonce_send:
                ...
                input_line = passwords.hash_password(hashed_password, self.nonce)[0]     
                self.nonce_send = False
        elif action_name == actions.OLD_PASSWORD_ACTION:
            input_line = getpass.getpass("Senha Antiga: ")          # get password (with no echo)
        elif action_name == actions.NEW_PASSWORD_ACTION:
            input_line = getpass.getpass("Nova Senha: ")          # get password (with no echo)
        elif action_name == actions.TYPE_ACTION:                    # get action type
            input_line = raw_input(">> ") 
        elif action_name == actions.MESSAGE_ACTION:
            input_line = raw_input("Mensagem: ")
